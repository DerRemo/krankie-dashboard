...
// App Store storefront codes: `asc reviews list` / `summarizations` return ISO alpha-3
// (USA/DEU), while `asc reviews ratings` (iTunes lookup) and the rest of the dashboard use
// alpha-2 (US/DE). Normalize everything to alpha-2 so the three tables share one code set.
//
// Complete ISO 3166-1 alpha-3 -> alpha-2 mapping (all 249 assigned codes).
...
export function toAlpha2(code: string): string {
  ...
  const mapped = ALPHA3_TO_2[up];
  if (mapped === undefined) {
    logger.warn({ phase: "reviews", code }, "unmapped territory code");
    return up;
  }
  return mapped;
}
